...
impl ItemList {
    ...
    pub async fn from(db_source : db::DB) -> Self {
        ItemList { items: db_source.get_all_items().await.unwrap() }
    }

    pub fn top(&mut self) -> &mut item::Item {
...
mod test {
    ...
    fn test_remove_top_quality_assess_add_item() {
        ...
        itemlist.put(Item::new("Test name 2".to_string(), "Test link 2".to_string()));
